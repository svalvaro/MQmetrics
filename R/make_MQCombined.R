...
make_MQCombined <- function(MQPathCombined,
    ...
    if ('#runningTimes.txt' %in% list.files(file.path(MQPathCombined, 'proc/'))) {
        ...
        alltables <- list(MQPathCombined,summary_table, peptides_table,
                          evidence_table,msscans_table, msScans_table,prot_groups,
                          modification_table, parameters_table, running_time)

        names(alltables) <- c('MQPathCombined','summary.txt', 'peptides.txt',
